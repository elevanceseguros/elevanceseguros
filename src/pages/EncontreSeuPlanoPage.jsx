...
const operadoras = [
  ...
  { nome: "MedSênior", path: "/medsenior", logo: "/logos/medsenior.webp", logoClass: "max-h-24 max-w-[140px] object-contain", cor: "border-emerald-200 hover:border-emerald-400", tag: "bg-emerald-100 text-emerald-700" },
];
...
export default function EncontreSeuPlanoPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    const detalhes = (pessoas || cidade) ? { pessoas: pessoas || undefined, cidade: cidade || undefined } : null;
    try {
      await fetch('https://n8n.srv1570723.hstgr.cloud/webhook/elevance-site-lead', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ nome, whatsapp, produto: 'Plano de Saúde', origem: window.location.pathname, detalhes }),
      });
    } catch (_) {}
    setSent(true);
  };
                    ...
                    <div key="form" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                      ...
                      <form onSubmit={handleSubmit} className="space-y-4 mt-4">
                        ...
                        <input name="whatsapp" type="tel" required placeholder="Ex: 11 99999-9999"
                          className="w-full bg-slate-50 border-none rounded-2xl py-4 px-6 text-sm outline-none focus:ring-2 focus:ring-blue-600 transition-all" />
                        <input name="cidade" type="text" placeholder="Sua cidade (opcional)"
                          className="w-full bg-slate-50 border-none rounded-2xl py-4 px-6 text-sm outline-none focus:ring-2 focus:ring-blue-600 transition-all" />
                        <div>
            ...
            {operadoras.slice(1).map((op) => (
              <Link key={op.nome} to={op.path}
                ...
                <div className="h-12 flex items-center justify-center">
                  <img src={op.logo} alt={op.nome} className={op.logoClass || "max-h-10 max-w-[140px] object-contain"} loading="lazy"
